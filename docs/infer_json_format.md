...
```json
{
  "proteinChain": {
    ...
    "templates": [
      {
        "mmcifPath": "msas/single_chain.cif",
        "queryIndices": [0, 1, 2],
        "templateIndices": [10, 11, 12]
      }
    ]
  },
}
...
* `templates`: A list of explicit single-protein-chain templates, each containing
  `mmcifPath` or inline `mmcif`, plus `queryIndices` and `templateIndices`.
  Paths resolve from the main JSON. Indices are zero-based complete polymer sequence
  positions, retaining unresolved residues. Multi-chain explicit CIFs, `chainId`
  and the old `templatesPath` field are rejected.
* Inline `pairedMsa` / `unpairedMsa` are also accepted; an explicit empty string
  suppresses that MSA channel and takes precedence over its path.
...
confidence in the selected JSON or NPZ format when enabled. Resume checks file
metadata only: it does not open or validate output contents or compare inputs,
model settings, or other conditions. Existing outputs can therefore be reused
after conditions change. Any missing or empty required file causes the entire
affected seed to rerun with all its samples; completed seeds and jobs are
preserved. Existing job-name and path-containment checks still apply.
`--skip false` recomputes all requested seeds.
